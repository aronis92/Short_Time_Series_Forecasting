...
class BHTARIMA(object):
    ...
    def _initilizer(self, T_hat, Js, Rs, Xs):
        # print('Ranks: ', Rs, np.asarray(Rs).shape)
        # initilize Us
        np.random.seed(0)
        U = [ np.random.random([j,r]) for j,r in zip( list(Js), Rs )]
        # U = [ np.float32(arr) for arr in U]
        # print('U shape: ', np.asarray(U).shape)
        # print('U0 shape: ', U[0].shape)
        # print('U1 shape: ', U[1].shape)
        # initilize es
        begin_idx = self._p + self._q
        np.random.seed(0)
        es = [ [ np.random.random(Rs) for _ in range(self._q)] for t in range(begin_idx, T_hat)]
        # print('Error shape: ', np.asarray(es).shape)
        return U, es
    ...
    def _update_Us(self, Us, Xs, unfold_cores, n, b_list):
        ...
        # all orth
            ...
            for t in range(begin_idx, T_hat):
                ...
                if c==0:
                    c+=1
                    tmp_unfold_X = copy.deepcopy(unfold_X)
                Bs.append(np.dot(np.dot(unfold_X, H.T), unfold_cores[t].T))
                
            b = np.sum(Bs, axis=0)
            # b_list.append(np.float32(b))
            # print(b)
            #b = b.replace(np.inf, np.nan).replace(-np.inf, np.nan).dropna()
            np.random.seed(0)
            U_, _, V_ = np.linalg.svd(np.float32(b), full_matrices=False)
            b_list.append(U_)
            b_list.append(V_)
            # print(np.dot(U_, V_))
            b_list.append(np.dot(U_, V_))
            Us[n] = np.dot(U_, V_)
        # only orth in J1
        ...
        # only orth in J2
            ...
            if n==1:
                ...
                temp = np.dot(a, b)
                Us[n] = temp / np.linalg.norm(temp)
        return Us, tmp_unfold_X, tmp_H, b_list
    
    def _update_Es(self, es, alpha, beta, unfold_cores, i, n):
        
        T_hat = len(unfold_cores)
        # print(len(unfold_cores), len(unfold_cores[0]), len(unfold_cores[0][0]))
        begin_idx = self._p + self._q
    ...
    def run(self):
        ...
        # new_core, new_X, b_list, tmp_H, tmp_X_unfold, Us_starting, result, loss, Xs, Xs_before_diff, Xs_after_diff, tmp_cores, tmp_Us, A_tmp, B_tmp, tmp_cores_up, Es_tmp, Es_start2 = self._run()
        result, loss, A_tmp = self._run()
        
        if self._convergence_loss:
            
            return result, loss, A_tmp#, Xs            
        
        return result, loss, A_tmp
        # return new_core, new_X, b_list, tmp_H, tmp_X_unfold, Us_starting, result, loss, Xs, Xs_before_diff, Xs_after_diff, tmp_cores, tmp_Us, A_tmp, B_tmp, tmp_cores_up, Es_tmp, Es_start2
    ...
    def _run(self):
        ...
        # trans_data = np.float32(trans_data)
        
        Xs = self._get_Xs(trans_data)
        # print('Before Differencing: ', len(Xs))
        Xs_before_diff = copy.deepcopy(Xs)

        if self._d!=0:
            begin, Xs = self._tensor_difference(self._d, Xs, 0)
        # print('After differencing: ', len(Xs))
        Xs_after_diff = copy.deepcopy(Xs)
        ...
        Us_starting = copy.deepcopy(Us)
        Es_start2 = copy.deepcopy(es)
        # print(Us[0].shape)
        # print(Us[1].shape)
        # print('Es :', len(es), len(es[0]), len(es[0][0]), len(es[0][0][0]))
        ...
        for k in range(self._K):
            ...
            for n in range(len(self._Rs)):
                ...
                #cores = [np.float32(arr) for arr in cores]
                tmp_cores_up.append(copy.deepcopy(cores))
                #if c==0:
                #    c += 1
                #    tmp_cores_up = copy.deepcopy(cores)
                
                # update Us 
                Us, tmp_X_unfold, tmp_H, b_list = self._update_Us(Us, Xs, unfold_cores, n, b_list)
                if c3 == 0:
                    #print("n: ", n)
                    c3 += 1
                    tmp_Us = copy.deepcopy(Us)
        ...
        return result, con_loss, A_tmp
        # return new_core, new_X, b_list, tmp_H, tmp_X_unfold, Us_starting, result, con_loss, Xs, Xs_before_diff, Xs_after_diff, tmp_cores, tmp_Us, A_tmp, B_tmp, tmp_cores_up, Es_tmp, Es_start2
