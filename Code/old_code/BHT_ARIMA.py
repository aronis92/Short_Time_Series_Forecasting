...
def inverse_MDT(X, r):
    ...
    #S_pseudo_inverse = np.linalg.pinv(S)
    return tl.tenalg.mode_dot( tl.unfold(X, 0), S_pseudo_inverse, -1)
...
def initialize_Es(ranks, q, T_hat, s):
    np.random.seed(0)
    r = list(ranks)
    r.append(q)
    r.append(T_hat - s)
    r = tuple(r)
    tmp = np.array([ [ np.random.random(ranks) for _ in range(q)] for t in range(s, T_hat)])
    return np.transpose(tmp, (2, 3, 1, 0))
...
def update_cores(m, p, q, Us, Xd, Gd, lam):
    temp_Gd = []
    ...
    for t in range(p + q, Xd.shape[-1]):
        ...
        for j in range(q):
            summary -= B[j] * tl.unfold(Es[..., j - 1, t - p - q], m)
...
def update_Um(m, p, d, q, Xd, Gd):
    ...
    # proc2 = tl.tenalg.proximal.procrustes(summary)
...
def update_Es(m, p, q, s, Gd, Es, A, B):
    ...
    for i in range(q):
        ...
        # _ = js.pop(outer_i)
        # for j in js:
        #     summary += B[j] * tl.unfold(Es[..., j, -(length + j + 1):(-j - 1)], m)
        ...
        As = []
        for t in range(p + q, Gd.shape[-1]):
            a = np.sum([A[ii] * tl.unfold(Gd[..., t - 1 - (ii + 1)], m) for ii in range(p)] , axis=0)
            
            b = np.sum([B[j] * tl.unfold(Es[..., -(j+1), t - p - q], m) for j in range(q) if j!=i ] , axis=0)
            As.append(tl.unfold(Gd[..., t], m) - a + b)
        E = np.sum(As, axis=0)
        for t in range(Es.shape[-1]):
            Es[..., i, t] = tl.fold(E / (2*(p + q - T_hat) * B[i]), m, Es[..., i, t].shape)
...
# Load Dataset
X = load_data(2)
#X, X_new = create_synthetic_data(p=2, q=2, d=1, dim=228, n_samples=100, mu=0, sigma=1)
# Set some variables
r = 5
p = 3
d = 2
...
if d>0:
    ...
    Xd = np.diff(X_hat, n = d, axis = -1)
else:
    Xd = X_hat

Es = initialize_Es(ranks, q, T_hat, s) # Initialize the random Es
Es_start = copy.deepcopy(Es[...,0,:])
...
while epoch < max_epoch and conv > threshold:
    ...
    rmse2 = np.sqrt( np.sum( (pred - X_test)**2 ) / np.size(X_test) )
